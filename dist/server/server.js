...
exports.showRooms = exports.getPort = exports.MyTCPServer = void 0;
...
class MyTCPServer {
    ...
    clientEndEvent(client) {
        ...
        if (this.room.port !== 8080) {
            ...
            if (this.roomCloseTimeoutID !== -1) {
                clearTimeout(this.roomCloseTimeoutID);
            }
    ...
    serverHallStateCommon(client) {
        client.on('data', this.serverHallStateDataEvent.bind(this, client));
    }
    // 监听大厅内的命令事件
    serverHallStateDataEvent(client, chunk) {
        let code = '';
        let param = '';
        ...
        switch (code) {
            ...
            // 用户进入房间
            case (commonCode_1.commonCode.JoinInfo): {
                ...
                if (roomName === this.room.roomName) {
                    for (let i = 0; i < this.room.users.length; i++) {
                        if (this.room.users[i][0] == userName) {
                            this.room.users[i][1].write(commonCode_1.commonCode.TwiceLoad);
                            break;
                        }
                    }
                }
                        ...
                        if (this.serverList[i].room.roomName == roomName) {
                            for (let j = 0; j < this.serverList[i].room.users.length; j++) {
                                if (this.serverList[i].room.users[i][0] == userName) {
                                    this.serverList[i].room.users[i][1].write(commonCode_1.commonCode.TwiceLoad);
                                    break;
                                }
                            }
                            break;
    ...
    serverRoomStateCommon(client) {
        client.on('data', this.serverRoomStateDataEvent.bind(this, client));
    }
    // 监听房间内的命令事件
    serverRoomStateDataEvent(client, chunk) {
        let code = '';
        let param = '';
        let content = chunk.toString();
        code = content.split(' ')[0];
                ...
                if (param2.indexOf('@') !== -1) {
                    ...
                    for (let i = 0; i < this.room.users.length; i++) {
                        if (s.indexOf(this.room.users[i][0]) == 0) {
                            ...
                            this.room.users[i][1].write(commonCode_1.commonCode.At + ' 该消息提到了你');
                            break;
                        }
                    }
                }
            ...
            case (commonCode_1.commonCode.Kick): {
                ...
                if (isRoot(rootName)) {
                    ...
                    if (kicClient === null) {
                        ...
                        kicClient.write(commonCode_1.commonCode.Kick);
                        // 房间人数减少
    ...
    startRollGame(currentTime) {
        ...
        for (let i = 0; i < this.rollGame.userNames.length; i++) {
            ...
            this.broadcast(commonCode_1.commonCode.At + ' ' + this.rollGame.userNames[i] + ' ' + nums[i]);
            if (nums[i] > maxNum) {
                maxNum = nums[i];
                count = 1;
                res = i;
        ...
        if (count === 1 || currentTime === 5) {
            if (currentTime === 5) {
                this.broadcast(commonCode_1.commonCode.At + ' 没有最高分，游戏结束');
            }
            else {
                this.broadcast(commonCode_1.commonCode.At + ' 获胜者是' + this.rollGame.userNames[res]);
            }
    ...
    cast(content, userName) {
        for (let i = 0; i < this.room.users.length; i++) {
            if (this.room.users[i][0] == userName) {
                this.room.users[i][1].write(commonCode_1.commonCode.At + ' ' + content);
                break;
            }
        }
    }
